...
- **Voice**: browser speech recognition or server transcription (OpenAI), spoken replies via OpenAI, ElevenLabs or Fish Audio neural voices (with free browser voices as a fallback), sentence-level streaming TTS, hands-free voice mode, animated listening / thinking / speaking states. Visitors can interrupt the assistant simply by talking over it (barge-in with echo filtering); the per-agent "Interruptible" switch turns this off.
- **Widget**: shadow-DOM embed (`widget.js`) that works on any site (HTML, WordPress, Shopify, Wix, Webflow, ...). Two styles: the full chat window, or a compact voice-only card (talk back and forth, with optional captions or no transcript at all). Launcher as a round button or a card with avatar and call-to-action ("Need help? Chat with Nicole"). Fully customisable: icon/logo, size, position, popup size, colours, fonts, radius, avatar, all texts, suggested questions, auto-open, branding. Live preview in the dashboard.
- **Appointment booking**: the assistant checks real availability and books during the conversation. Opening hours, services with durations, capacity, buffers, notice and closed dates; 18 business-type presets (spa, medical, dental, salon, barber, therapy, veterinary, restaurant, fitness, automotive, home services, legal, property, tutoring, consulting, photography, tattoo, general) that seed the services and the questions to ask. Link any calendar by its secret ICS address so busy times are never offered, and subscribe to a per-agent feed so bookings land in Google, Outlook or Apple Calendar. Confirmation emails to both sides, plus a public page where the visitor can add the appointment to their calendar or cancel.
- **Dashboard**: onboarding wizard, agents, knowledge, conversations & transcripts, leads, bookings, unanswered questions ("teach answer"), analytics, settings, billing.
